...
/*   Updated: 2024/10/15 14:20:25 by nde-chab         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	make_join(t_parsing *parsing, char *value, char *rest,
		int *position)
{
	char	*temp;
	...
	else if (handle_quote(parsing->input, 0, *position) == 1)
	{
		...
		*position = ft_strlen(temp);
		...
		return (make_join_2(parsing, value, rest, position));
}
